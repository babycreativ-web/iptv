    ...
    <footer style={{ padding: '80px 0 40px', background: '#050614', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
      <div className="container">
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '50px', marginBottom: '60px' }}>
          ...
          <div>
            <h4 style={{ marginBottom: '25px' }}>{getFooterText('LEGAL', 'LEGAL', 'LÉGAL')}</h4>
            <ul style={{ listStyle: 'none', color: 'var(--text-gray)', fontSize: '14px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
              <li>{getFooterText('Privacy Policy', 'Política de privacidad', 'Politique de confidentialité')}</li>
              <li>{getFooterText('Terms of Service', 'Términos de servicio', 'Conditions générales de vente')}</li>
              <li>{getFooterText('Refund Policy', 'Política de reembolso', 'Remboursement')}</li>
            </ul>
          </div>
          <div>
            <h4 style={{ marginBottom: '25px' }}>{getFooterText('SECURE PAYMENT', 'PAGO SEGURO', 'PAIEMENT SÉCURISÉ')}</h4>
        ...
        <div style={{ textAlign: 'center', borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: '40px', color: 'var(--text-gray)', fontSize: '12px' }}>
          &copy; 2026 4K SMART IPTV. {getFooterText('ALL RIGHTS RESERVED.', 'TODOS LOS DERECHOS RESERVADOS.', 'TOUS DROITS RÉSERVÉS.')}
        </div>
      </div>
    </footer>
  );
